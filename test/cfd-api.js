...
describe('cfd-api.js', function () {
  ...
  const newCFDInitiated = async (
    ...
    await proxyApi.proxyDeposit(counterpartyProxy, cfd, notionalAmountDai)
    return cfd
  }
  ...
  it('check cfdPartyIsBuyer contract details', async () => {
    ...
    const buyerLiquidationPrice = new BigNumber(
      stkPrice - (stkPrice * liquidationPercentage) / 100
    ).toFixed(7)

    assertEqualAddress(
      cfd.details.address,
      cfdPartyIsBuyer.options.address,
      'Wrong address value'
    )
    ...
    assertEqualAddress(
      cfd.details.buyer,
      partyProxy.options.address,
      'Wrong buyer value'
    )
    assert.equal(
      cfd.details.buyerIsSelling,
      false,
      'Wrong buyerIsSelling value'
    )
    assert.equal(cfd.details.market, marketStr, 'Wrong market value')
    assert.equal(
      cfd.details.notionalAmountDai,
      daiUsed,
      'Wrong notionalAmountDai value'
    )
    assert.equal(
      cfd.details.buyerInitialNotional,
      daiUsed,
      'Wrong buyerInitialNotional value'
    )
    assert.equal(cfd.details.strikePrice, price, 'Wrong strikePrice value')
    assert.equal(
      cfd.details.buyerDepositBalance.toFixed(),
      daiUsed,
      'Wrong buyerDepositBalance value'
    )
    assert.equal(
      cfd.details.buyerInitialStrikePrice,
      price,
      'Wrong buyerInitialStrikePrice value'
    )
    assert.equal(
      cfd.details.buyerLiquidationPrice.toFixed(),
      buyerLiquidationPrice,
      'Wrong buyerLiquidationPrice value'
    )
  })
  ...
  it('check cfdPartyIsSeller contract details', async () => {
    ...
    const daiUsed = 1
    const stkPrice = parseFloat(price)
    const sellerLiquidationPrice = new BigNumber(
      stkPrice + (stkPrice * liquidationPercentage) / 100
    ).toFixed(7)

    assertEqualAddress(
      cfd.details.address,
      cfdPartyIsSeller.options.address,
      'Wrong address value'
    )
    assert.equal(cfd.details.closed, false, 'Wrong closed value')
    assert.equal(cfd.details.status, 1, 'Wrong status value')
    ...
    assert.equal(
      cfd.details.sellerIsSelling,
      false,
      'Wrong sellerIsSelling value'
    )
    assert.equal(cfd.details.market, marketStr, 'Wrong market value')
    assert.equal(
      cfd.details.notionalAmountDai,
    ...
    assert.equal(
      cfd.details.sellerInitialNotional,
      daiUsed,
      'Wrong sellerInitialNotional value'
    )
    assert.equal(cfd.details.strikePrice, price, 'Wrong strikePrice value')
    assert.equal(
      cfd.details.sellerDepositBalance.toFixed(),
      daiUsed,
      'Wrong sellerDepositBalance value'
    )
    assert.equal(
      cfd.details.sellerInitialStrikePrice,
      price,
      'Wrong sellerInitialStrikePrice value'
    )
    assert.equal(
      cfd.details.sellerLiquidationPrice.toFixed(),
      sellerLiquidationPrice,
      'Wrong sellerLiquidationPrice value'
    )
  })
  ...
  it('change strike price', async () => {
    ...
    assert.equal(updatedCfd.details.strikePrice, newPrice, 'Wrong strike price')
  ...
  it('deposit joins new party and initiates the contract', async () => {
    ...
    assert.equal(
      await cfd.methods.seller().call(),
      counterpartyProxy.options.address,
      'Wrong seller account'
    )
    assert.equal(
      await cfd.methods.notionalAmountDai().call(),
  ...
  it('getCFD gets contract details', async () => {
    ...
    assertEqualAddress(
      cfdDetailed.details.buyer,
      buyerProxy.options.address,
      'Wrong buyer account'
    )
    assertEqualAddress(
      cfdDetailed.details.seller,
      EMPTY_ACCOUNT,
      'Wrong seller account'
    )
    assert.equal(
  ...
  it('sale flow with sellCFD and buyCFD faciliates a sale', async () => {
    ...
    assert.equal(
      await cfd.methods.buyer().call(),
      thirdPartyProxy.options.address,
      `buyer should now be the thirdPartyProxy`
    )
  ...
  describe('contractsForParty', function () {
    ...
    it('returns only current open contracts when default options', done => {
      callAndAssert(partyProxy.options.address, {}, cfds => {
        ...
        assertEqualAddress(
          cfds[0].details.address,
          cfdPartyIsBuyer.options.address,
          `buyer`
        )
        assertEqualAddress(
          cfds[1].details.address,
          cfdPartyIsSeller.options.address,
          `seller`
        )
        done()
    ...
    it('includes liquidated/closed contracts if requested', done => {
      callAndAssert(
        ...
        cfds => {
          ...
          assertEqualAddress(
            cfds[2].details.address,
            cfdLiquidated.options.address
          )
          done()
        }
      )
  ...
  describe('changeSaleCFD', function () {
    it('change sale price for a CFD for sale', async () => {
      ...
      await api.changeSaleCFD(
        cfd.options.address,
        buyerProxy,
        parseFloat(price) * 2
      ) // Set the sale strike price as double
      const updatedCfd = await api.getCFD(cfd.options.address)

      assert.equal(
        updatedCfd.details.buyerSaleStrikePrice.toFixed(5),
        parseFloat(price) * 2,
        'Wrong buyerSaleStrikePrice value'
      )
    })
  })
  ...
  describe('topup & withdraw', function () {
    ...
    it('topup a CFD', async () => {
      ...
      assert.equal(
        currentCfd.details.buyerDepositBalance.toNumber(),
        1,
        'Initial buyerDepositBalance is wrong'
      )
      ...
      assert.equal(
        newCfd.details.buyerDepositBalance.toNumber(),
        3,
        'Initial buyerDepositBalance is wrong'
      )
    })
